...
- **src/**: 21-module flat Python stdlib library — all modules at `src/*.py` (db, router, confidence, interview, worktree, circuit_breaker, handoff, sync, documents, git_utils, response_analyzer, progress, exit_tracker, loop, experiment, convergence, fitness, rtk_integration, self_assessment, session_miner, task_graph)
- **hooks/**: 8 Python hooks that fire on Claude Code lifecycle events
- **skills/**: 12 markdown skill definitions (5-section format)
- **agents/**: 12 agent personas (research, review, implementation, orchestration)
- **commands/**: 15 slash commands (user-facing entry points)
